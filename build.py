...
class MuseumSiteGenerator:
    ...
    def get_related_records(self, conn, record_id, esport, team, player):
        """Get related records"""
        cursor = conn.cursor()
        related = cursor.execute("""
            ...
            FROM records r
            WHERE id != ? 
            AND (esport = ? OR team = ? OR player = ?)
            ORDER BY RANDOM()
            LIMIT 4
        """, (record_id, esport or '', team or '', player or '')).fetchall()
    ...
    def get_filtered_records(self, conn, category='all', esport='all', era='all'):
    ...
    def generate_browse_pages(self, conn):
        ...
        categories = ['all', 'jerseys', 'hardware', 'signatures']
        esports = ['all', 'cod', 'halo', 'cs', 'valorant', 'lol', 'rl']
        ...
        for category in categories:
            for esport in esports:
                for era in eras:
                    if category == 'all' and esport == 'all' and era == 'all':
                        continue  # Already generated
                    
                    records = self.get_filtered_records(conn, category, esport, era)
